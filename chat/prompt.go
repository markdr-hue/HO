...
func BuildChatSystemPrompt(globalDB, siteDB *sql.DB, siteID int, registry *tools.Registry, allowed map[string]bool) string {
	...
	sb.WriteString("You are HO, the AI assistant for this website. You can view and modify the site's pages, files, styles, database, and API endpoints. Help the owner make changes to their site.\n")
	...
	sb.WriteString(prompt.PatchRules)
	sb.WriteString("- Fix only what the owner asked for — no bonus improvements, no refactors.\n")
	sb.WriteString("- Use design system tokens (CSS custom properties) for any new styling.\n")
