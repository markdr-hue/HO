...
func RegisterRoutes(r chi.Router, deps *Deps, adminFS fs.FS) {
	...
	tunnelH := &TunnelHandler{manager: deps.TunnelManager}
	templatesH := &TemplatesHandler{deps: deps}
	...
	r.Route("/admin/api", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/templates/{templateID}", templatesH.Get)
			...
			r.Route("/sites/{siteID}", func(r chi.Router) {
				// Tunnel sharing — all authenticated users can view status
				r.Get("/tunnel/status", tunnelH.Status)
				...
				r.Get("/tables/{tableName}/csv", siteTablesH.ExportCSV)
				r.Get("/endpoints", siteEndpointsH.List)
				r.Get("/endpoints/{endpointID}", siteEndpointsH.Get)
				r.Get("/files", siteFilesH.List)
				r.Get("/files/{fileID}/content", siteFilesH.Content)
				...
				r.Get("/pages/{pageID}/versions", sitePagesH.ListVersions)
				r.Get("/assets/{assetID}/versions", siteAssetsH.ListVersions)
				r.Get("/layouts/{layoutID}/versions", siteLayoutsH.ListVersions)
				...
				r.Group(func(r chi.Router) {
					...
					r.Post("/tunnel/start", tunnelH.Start)
					r.Post("/tunnel/stop", tunnelH.Stop)
					...
					r.Post("/secrets", siteSecretsH.Create)
					r.Delete("/secrets/{secretID}", siteSecretsH.Delete)

					r.Delete("/layouts/{layoutID}", siteLayoutsH.Delete)
					r.Put("/layouts/{layoutID}", siteLayoutsH.Update)
					r.Post("/pages/{pageID}/revert/{version}", sitePagesH.Revert)
					r.Post("/assets/{assetID}/revert/{version}", siteAssetsH.Revert)
					r.Post("/layouts/{layoutID}/revert/{version}", siteLayoutsH.Revert)
			...
			r.Group(func(r chi.Router) {
				...
				// Sites (create, clone from template)
				r.Post("/sites", sites.Create)
				r.Post("/sites/clone", templatesH.Clone)

				// Templates (save, delete)
				r.Post("/templates", templatesH.Save)
				r.Delete("/templates/{templateID}", templatesH.Delete)
