...
func (h *SiteEndpointsHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	wsRows, err := siteDB.Query(
		`SELECT id, path, write_to_table, room_column, requires_auth, created_at
		 FROM ho_ws_endpoints ORDER BY path ASC`,
	)
	if err == nil {
		defer wsRows.Close()
		for wsRows.Next() {
			var e siteEndpoint
			e.SiteID = siteID
			var writeToTable, roomCol string
			if err := wsRows.Scan(&e.ID, &e.Path, &writeToTable, &roomCol, &e.RequiresAuth, &e.CreatedAt); err != nil {
				continue
			}
			e.Methods = `["WS"]`
			e.TableName = "WebSocket"
			if writeToTable != "" {
				e.TableName = "WebSocket → " + writeToTable
			}
			e.PublicRead = !e.RequiresAuth
			e.ID = e.ID + 200000
			endpoints = append(endpoints, e)
	...
	// Include SSE stream endpoints.
	streamRows, err := siteDB.Query(
		`SELECT id, path, requires_auth, created_at
		 FROM ho_stream_endpoints ORDER BY path ASC`,
	)
	if err == nil {
		...
		for streamRows.Next() {
			var e siteEndpoint
			e.SiteID = siteID
			if err := streamRows.Scan(&e.ID, &e.Path, &e.RequiresAuth, &e.CreatedAt); err != nil {
				continue
			}
			e.Methods = `["SSE"]`
			e.TableName = "Stream"
	...
	uploadRows, err := siteDB.Query(
		`SELECT id, path, table_name, max_size_mb, requires_auth, created_at
		 FROM ho_upload_endpoints ORDER BY path ASC`,
	)
	if err == nil {
		...
		for uploadRows.Next() {
			...
			var maxSize int
			if err := uploadRows.Scan(&e.ID, &e.Path, &tblName, &maxSize, &e.RequiresAuth, &e.CreatedAt); err != nil {
				continue
			}
			e.Methods = `["POST"]`
			e.TableName = "Upload"
			if tblName.Valid && tblName.String != "" {
				e.TableName = "Upload → " + tblName.String
			}
			e.PublicRead = !e.RequiresAuth
			e.ID = e.ID + 400000
			endpoints = append(endpoints, e)
