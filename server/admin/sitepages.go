...
func (h *SitePagesHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	res, err := siteDB.ExecWrite(
		`INSERT INTO ho_pages (path, title, content, status, metadata)
		 VALUES (?, ?, ?, 'published', '{}')`,
		req.Path, title, content,
	)
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE constraint") {
			writeError(w, http.StatusConflict, fmt.Sprintf("page %q already exists", req.Path))
			return
		}
	...
	writeJSON(w, http.StatusCreated, map[string]interface{}{"id": id, "path": req.Path})
}
