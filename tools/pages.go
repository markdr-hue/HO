...
func (t *PagesTool) Guide() string {
	return `### manage_pages
- Content = main content only (no DOCTYPE/html/head/body — see Platform Rules).
- Use var(--color-*) and classes from the global CSS. No <style> blocks in pages.
- Pages must be functional: fetch from APIs, handle forms, wire interactive elements.
- For complex JS, create .js files via manage_files(scope="page") and list them in the page's assets array.
