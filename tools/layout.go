...
- template: the complete HTML shell between <body> and </body> with a {{content}} marker where page content is injected.
  The server replaces {{content}} with each page's HTML.
  Traditional site: <header>...</header><main>{{content}}</main><footer>...</footer>
  Chromeless app (game, canvas, SPA): <main>{{content}}</main>
- head_content: extra HTML for <head> (fonts, meta tags, CDN links). Shared CSS/JS auto-injected — don't duplicate.
- Do NOT put <style> blocks in the template — all CSS belongs in the global stylesheet.
