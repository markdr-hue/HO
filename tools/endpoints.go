...
func (t *EndpointsTool) Description() string {
	return "Manage endpoints. CRUD: create_api. Auth: create_auth. AI: create_llm. Real-time: create_websocket/create_stream. Files: create_upload. Also: create_oauth, generate_docs. Each type supports create/update/list/delete actions."
}
...
- **create_websocket**: Stateless message relay — see WebSocket Architecture in Platform Contracts for detailed patterns. Connection: ws(s)://host/api/{path}/ws?room=ROOMNAME.
...
- Get current user: fetch('/api/{path}/me', {headers: {'Authorization': 'Bearer ' + localStorage.getItem('auth_token')}}) -> user object. Always check token exists first: if (!localStorage.getItem('auth_token')) return null;
- Auth header: {headers: {'Authorization': 'Bearer ' + localStorage.getItem('auth_token')}}
- JWT tokens are opaque strings — NEVER JSON.parse them. Parse the /me response, not the token.
- WARNING: Do NOT use Supabase, Firebase, Auth0, or any external auth SDK. No Auth.getUser(), no createClient(). Use ONLY the fetch patterns above.
