...
// HasParameterizedRoutes returns true if any page path contains a :param segment.
// Implements prompt.PlanContext.
func (p *Plan) HasParameterizedRoutes() bool {
	for _, pg := range p.Pages {
		if strings.Contains(pg.Path, ":") {
			return true
		}
	}
	return false
...
func ResetPipeline(sdb *db.SiteDB) error {
	_, err := sdb.ExecWrite(`UPDATE ho_pipeline_state SET
		stage = 'EXPAND', plan_json = NULL, tool_calls_completed = 0,
...
// ---------------------------------------------------------------------------
// CDN library validation & version resolution
// ---------------------------------------------------------------------------

// cdnPkgPattern matches npm-based CDN URLs: jsdelivr, unpkg, esm.sh
// Captures: (1) full host+path prefix, (2) package name (may be scoped), (3) version, (4) file path suffix
var cdnPkgPattern = regexp.MustCompile(
	`^(https?://(?:cdn\.jsdelivr\.net/npm|unpkg\.com|esm\.sh)/)(@?[a-z0-9][\w.\-]*/?)@([^/]+)(/.*)?$`,
)

// validateExternalLibraries checks each CDN URL in the plan and attempts to
// resolve broken or @latest URLs to a pinned version. It mutates plan.ExternalLibraries
// in-place. Returns informational messages (not errors — never blocks the build).
func validateExternalLibraries(plan *Plan, logger *slog.Logger) []string {
	...
	for i := range plan.ExternalLibraries {
		lib := &plan.ExternalLibraries[i]
		url := lib.URL

		// 1. If URL contains @latest, resolve to pinned version first.
		if m := cdnPkgPattern.FindStringSubmatch(url); m != nil && m[3] == "latest" {
			pkg := strings.TrimSuffix(m[2], "/")
			if ver, err := resolveLatestNpmVersion(client, pkg); err == nil && ver != "" {
				newURL := m[1] + m[2] + "@" + ver + m[4]
				if headCheckOK(client, newURL) {
					lib.URL = newURL
					msgs = append(msgs, fmt.Sprintf("pinned %s: @latest → @%s", lib.Name, ver))
					logger.Info("CDN: pinned @latest to version", "lib", lib.Name, "version", ver)
					continue
				}
			}
		}

		// 2. HEAD-check the current URL.
		if headCheckOK(client, url) {
			logger.Info("CDN: URL validated", "lib", lib.Name, "url", url)
			continue
		}

		// 3. URL is broken — try to resolve the latest version.
		if m := cdnPkgPattern.FindStringSubmatch(url); m != nil {
			pkg := strings.TrimSuffix(m[2], "/")
			if ver, err := resolveLatestNpmVersion(client, pkg); err == nil && ver != "" {
				newURL := m[1] + m[2] + "@" + ver + m[4]
				if headCheckOK(client, newURL) {
					lib.URL = newURL
					msgs = append(msgs, fmt.Sprintf("fixed %s: resolved to @%s", lib.Name, ver))
					logger.Info("CDN: resolved broken URL", "lib", lib.Name, "old", url, "new", newURL)
					continue
				}
			}
		}

		// 4. Could not fix — warn but don't block.
		msgs = append(msgs, fmt.Sprintf("warning: could not validate CDN URL for %s: %s", lib.Name, url))
		logger.Warn("CDN: URL unreachable, keeping as-is", "lib", lib.Name, "url", url)
	}

	return msgs
}

// headCheckOK sends an HTTP HEAD (falling back to GET) and returns true if status is 200.
func headCheckOK(client *http.Client, url string) bool {
	...
	// Some CDNs don't support HEAD; fall back to GET on 405.
...
// jsdelivrPkgResponse is the subset of the jsdelivr API response we need.
type jsdelivrPkgResponse struct {
	Tags map[string]string `json:"tags"` // e.g. {"latest": "0.172.0"}
}

// resolveLatestNpmVersion queries the jsdelivr API for the latest version of an npm package.
func resolveLatestNpmVersion(client *http.Client, pkg string) (string, error) {
	apiURL := "https://data.jsdelivr.com/v1/packages/npm/" + pkg
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("jsdelivr API returned %d", resp.StatusCode)
	}
	...
	var pkg2 jsdelivrPkgResponse
	if err := json.Unmarshal(body, &pkg2); err != nil {
		return "", err
	}

	if v := pkg2.Tags["latest"]; v != "" {
		return v, nil
	}
	return "", fmt.Errorf("no 'latest' tag found for %s", pkg)
}
