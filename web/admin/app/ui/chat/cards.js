...
export function createQuestionCard(questionData) {
  const { id, question, urgency, context, type, options, site_id: siteId } = questionData;
  ...
  if (isSingleChoice) {
    ...
    btns.forEach(btn => {
      btn.addEventListener('click', () => submitAnswer(id, btn.textContent, card, siteId));
    });
    ...
    const submitBtn = h('button', {
      ...
      onClick: () => { if (getValue()) submitAnswer(id, getValue(), card, siteId); },
    }, 'Submit');
    card.appendChild(inputEl);
    ...
    if (input) {
      input.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && getValue()) submitAnswer(id, getValue(), card, siteId);
      });
    }
...
async function submitAnswer(questionId, answer, cardEl, siteId) {
  try {
    await post(`/admin/api/questions/${questionId}/answer`, { answer });

    // Decrement badge count for this site's answered question
    if (siteId) {
      const map = Object.assign({}, state.get('pendingQuestions') || {});
      map[siteId] = Math.max(0, (map[siteId] || 0) - 1);
...
export function createQuestionGroup(questions) {
  ...
  async function submitAll() {
    ...
    try {
      ...
      // Clear badge — decrement for this site's submitted questions
      const groupSiteId = items[0]?.data?.site_id;
      if (groupSiteId) {
        const map = Object.assign({}, state.get('pendingQuestions') || {});
        map[groupSiteId] = Math.max(0, (map[groupSiteId] || 0) - succeeded);
        state.set('pendingQuestions', map);
      }
