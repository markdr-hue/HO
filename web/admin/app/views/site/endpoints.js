...
function renderApiCard(ep, siteId, container) {
  const methods = safeJsonParse(ep.methods, []);
...
function showEditModal(container, ep, siteId) {
  let methods = safeJsonParse(ep.methods, []);

  // Method toggles
  const methodChecks = ALL_METHODS.map(m => {
    const cb = h('input', { type: 'checkbox', checked: methods.includes(m) });
    return h('label', { className: 'toggle', style: { marginRight: '8px' } }, [cb, m]);
  });
  ...
  const form = h('div', {}, [
    ...
    h('div', { className: 'form-group mb-3' }, [
      h('label', { className: 'toggle' }, [authCheck, 'Requires Authentication']),
    ]),
    h('div', { className: 'form-group mb-3' }, [
      h('label', { className: 'toggle' }, [publicReadCheck, 'Public Read (GET without auth)']),
    ]),
    h('div', { className: 'form-group mb-3' }, [
...
async function showCreateEndpointModal(container, siteId) {
  ...
  const methodChecks = {};
  const methodsRow = h('div', { className: 'flex gap-3', style: { flexWrap: 'wrap' } },
    ['GET', 'POST', 'PUT', 'DELETE'].map(m => {
      const cb = h('input', { type: 'checkbox', checked: true });
      methodChecks[m] = cb;
      return h('label', { className: 'toggle text-sm' }, [cb, m]);
    })
  );
  ...
  const form = h('div', {}, [
    ...
    h('div', { className: 'form-group' }, [h('label', { className: 'toggle' }, [authCheck, 'Requires Auth'])]),
    h('div', { className: 'form-group' }, [h('label', { className: 'toggle' }, [publicReadCheck, 'Public Read (GET without auth)'])]),
